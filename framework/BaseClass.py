...
from utils import get_activation_scheme, input_validation
...
class FullyConnectedNetwork(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.input_layer = nn.Linear(network_params['input_layer']['units'], \
            network_params['hidden_layer'][0]['units'])
        self.hidden_layers = []
        for i in range(1, len(network_params['hidden_layer'])):
            self.hidden_layers.append(nn.Linear(network_params['hidden_layer'][i-1]['units'],\
                network_params['hidden_layer'][i]['units']))
        self.output_layer = nn.Linear(network_params['hidden_layer'][-1]['units'],\
